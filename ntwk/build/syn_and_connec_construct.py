...
def connections_from_pairs(NTWK, Pairs, pre_name, post_name):
    """

    set connections from a list of pairs:
    from a list 
    [
        [1,3],
        [1,7],
    ...
    ii = np.flatnonzero(NTWK['POPULATIONS']==pre_name)[0]   # pre pop. index
    jj = np.flatnonzero(NTWK['POPULATIONS']==post_name)[0]  # post pop. index

    if (len(Pairs)>0) and NTWK['M'][ii,jj]['Q']!=0:
        ...
        connection = brian2.Synapses(NTWK['POPS'][ii], NTWK['POPS'][jj], 
                                     model='w:siemens', on_pre=on_pre)
        connection.connect(i=np.array(Pairs)[:,0], j=np.array(Pairs)[:,1]) 
        connection.w = NTWK['M'][ii,jj]['Q']*brian2.nS

        return connection

    else:

        print('\n [!!]', pre_name, post_name, '-> 0  weight connection [!!] \n')
        return None
