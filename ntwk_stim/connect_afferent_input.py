...
def set_spikes_from_time_varying_rate_synchronous(time_array, rate_array, N, SEED=1):
    ...
    for it in range(len(time_array)):
        rdm_num = np.random.random(N)
        for ii in np.arange(N)[rdm_num<DT*rate_array[it]*1e-3]:
            indices.append(ii) # all the indices
            times.append(time_array[it]) # all the same time !
...
def construct_feedforward_input_synchronous(NTWK, target_pop,
    ...
    N_per_cell = int(afferent_pop['pconn']*afferent_pop['N'])
    i_rdms = np.concatenate([\
                             np.random.choice(np.arange(afferent_pop['N']), N_per_cell)\
                             for jjj in range(target_pop.N)])
    j_fixed = np.concatenate([np.ones(N_per_cell,dtype=int)*jjj for jjj in range(target_pop.N)])
